...
**Step 3.2: Generate Explanations (Bedrock / Local AI Explainability)**

**Presenter Script:**
"Our explainability layer generates coaching explanations for each alert. It can only reference wellness and development resources—never punitive HR actions. All explanations are deterministic (no black-box AI), rule-based, and transparent."

**Action (Local Demo Mode):**
```bash
# Open the AI explanations JSON generated by the demo
cat artifacts/local_demo_*/05_ai_explanations.json | jq '.[] | select(.alertType == "burnout") | .summary' | head -1
```

**Action (AWS Mode with Bedrock):**
```bash
...
**Verification:**
- ✅ Explanation generated and saved (JSON file or S3 object)
- ✅ Explanation JSON contains "summary", "why_flagged", and "next_best_actions" fields

**Continue condition:** ✅ → proceed to State 4
**STOP condition:** Explanation unavailable → switch to Fallback Plan (pre-recorded explanations)

**Time budget:** 1 min

---

### ⭐ WOW MOMENT: AI Explainability Output (NEW)

**THIS IS THE HEADLINE DEMO MOMENT — Read This Aloud to Judges**

**Presenter Script (60 seconds):**
"This is our AI explainability layer. Notice it's 100% rule-based—no LLM black box. We generate human-readable explanations that managers can act on immediately. Here's an example:"

**Display Example (From Real Demo Output):**
...
**Key talking points (read aloud):**
1. **No raw data exposed:** No email content, no calendar details—only signal counts and computed features
2. **Transparent reasoning:** "Why flagged" shows exactly which signals triggered the alert (meetings, messages, context switching)
3. **Actionable next steps:** Manager knows exactly what to do: "Schedule 1:1", "Review calendar", etc.
4. **No punitive language:** We never say "performance issue" or "consider discipline"—only wellness and development
5. **Deterministic:** Run the same scenario 100 times, get identical explanations (no LLM randomness)

**Action (Local Demo):**
1. Open file: `cat artifacts/local_demo_*/05_ai_explanations.json | jq '.[] | select(.alertType == "burnout")'`
2. Read the summary aloud (⬆️ above)
3. Point to "Why flagged" and highlight 1–2 reasons
4. Point to "Next best actions" and say: "The manager doesn't need AI to tell them this is a problem. They need help knowing what to do. That's what we deliver."

**Why this wins the hackathon:**
- ✅ Explainability (judges care about AI transparency)
- ✅ Privacy (only signals, no raw text)
- ✅ Determinism (no LLM randomness)
- ✅ Actionability (concrete manager actions)
- ✅ No Bedrock required (rule-based fallback if LLM unavailable)

**Time for this segment:** 2–3 minutes (worth every second)
